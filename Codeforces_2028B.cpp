...
#define ISBITSET(x,idx) (((1LL<<idx)&x)>0LL)
...
bool Check(ll n, vector<pair<ll,ll>>& a, ll mid){
    ll mx{0};
    ll mn{0};
    LOOP(0,n){
        mn=max(a[i].first,mn-mid);
        mx=min(a[i].second,mx+mid);
        if(mx<mn){
            return false;
        }
    }
...
void solve(ll n, ll b, ll c) {
    if(b==0){
        cout << (n>c+2?-1:(c<=n-1?n-1:n)) << endl;
        return;
    }
    ll k = ceil(n-c,b);
    cout << n-k << endl;
