...
void DFS(vector<vll>& g, vector<ll>& ans, vll& scores, ll node, ll mn, ll mx){
    if(ans[node-1]!=LONG_LONG_MIN){
        return;
    }
    for (auto& x:g[node]){
        ans[node-1]=scores[node-1]-min(mn, 0LL);
        DFS(g, ans, scores, x, scores[node-1]-max(mx, 0LL), ans[node-1] );
    }
}
