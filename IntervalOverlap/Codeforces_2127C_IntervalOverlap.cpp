...
void solve(ll n, vll& a, vll& b) {
    ...
    ll ans{0};
    LOOP(0,n){
        rng[i].first = min(a[i], b[i]);
        rng[i].second = max(a[i], b[i]);
        ans+=rng[i].second-rng[i].first;
    }
    sort(rng);
    ll inc{LONG_LONG_MAX};
    LOOP(1,n){
        inc = min(inc, rng[i].first-rng[i-1].second);
    }
    cout << ans + 2*max(inc, 0LL) << endl;
}
