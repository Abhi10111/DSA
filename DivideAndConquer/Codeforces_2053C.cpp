...
pair<ll, ll> DivideAndConquer(ll l, ll k){
    if(l<k){
        return {0,0};
    }
    ll m = l/2;
    auto rec = DivideAndConquer(m, k);
    ll s = rec.second+m*rec.second*rec.first;
    if(l%2){
        return  {2*rec.first+1, s+m};
    }
    else{
        return {2*rec.first, s};
    }
}
void solve(ll n, ll k){
    cout << DivideAndConquer(n, k) << endl;
}

void solve(ll k, ll q, vll& a, vll& b, vll& Q){
}
