...
ll GetSum(string s, ll st, ll en){
    ll su{0};
    LOOP(st, en+1){
        su+=s[i]-'0';
    }
...
void solve(ll n, vector<string>& a) {
    ...
    for(auto& x: a){
        ...
        f[x.size()][GetSum(x, 0, x.size()-1)]++;
    }    
    for(auto& x:a){
        ll tot=GetSum(x, 0, x.size()-1);
        LOOP(1, x.size()+1){
            if((i%2)==(x.size()%2)){
                ll rSum=GetSum(x, (x.size()-i)/2, x.size()-1);
                ans+=f[i][2*rSum-tot];
            }
        }
        LOOP(1, x.size()){
            if((i%2)==(x.size()%2)){
                ll rSum=GetSum(x, 0, (x.size()+i)/2-1);
                ans+=f[i][2*rSum-tot];
            }
        }
    }
