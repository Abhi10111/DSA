...
void solve(ll n, vector<pair<ll,ll>>& a) {
    vector<std::set<ll>> f(2);
    ll ans{0};
    for(auto& x:a){
        f[x.second].insert(x.first);
    }
    for(auto& x:a){
        if(f[(x.second+1)%2].count(x.first)){
            ans+=f[x.second].size()-1LL;
        }
        if(f[(x.second+1)%2].count(x.first-1) && f[x.second].count(x.first-2)){
            ans++;
        }
    }
    cout << ans << endl;
